...
sudo nmap - sC - sV - oA jab\_scan 10.10.11.4

![](/src/assets/images/jab/nmap1.png)![](/src/assets/images/jab/nmap2.png)![](/src/assets/images/jab/nmap3.png)
...
![](/src/assets/images/jab/pid2.png)
...
![](/src/assets/images/jab/pid3.png)

Go to Advanced and Add the ip address as connect server: 10.10.11.4

![](/src/assets/images/jab/pid4.png)Now, we get an XMPP Client Registration page

![](/src/assets/images/jab/pid5.png)After getting the registration successful prompt, we proceed to the 'Buddy List'. We click on 'Join a Chat', then 'Room List'. We get a popup to query conference.jab.htb, proceed and now we het two rooms: test and test2![](/src/assets/images/jab/pid7.png)We check these two rooms out by clicking 'Join'. We see that we can only join the "test 2" room![](/src/assets/images/jab/pid8.png)Nothing interesting here, other than the name: ﻿bdavis. Looking aroud, we see a feature that helps us search for users

![](/src/assets/images/jab/pid9.png)Click Search Directory

![](/src/assets/images/jab/pid10.png)Enter "\*" for wildcard query and click on OK

![](/src/assets/images/jab/pid11.png)Now, we get a list of users as the output

![](/src/assets/images/jab/pid12.png)We cant copy and paste this so we can try export the search users result to a list. To do this we need to open another instance of pidgin and send the ouput to a list:

sudo pidgin -d > users.txt

CTRL C to make sure the changes are exported to the list. We have out result, but its jumbled:

![](/src/assets/images/jab/pid13.png)However, we can clean this. Looking at the format, we are interested in the names between the value tag. Ex: \<value>[lmccarty@jab.htb](mailto\:lmccarty@jab.htb)\</value>

grep -o '\<value>.\*\</value>' users.txt | cut -d ">" -f 2 | cut -d "@" -f 1 | cut -d "\<" -f 1 | tee new\_users

![](/src/assets/images/jab/cut.png)Remove the first three lines and you're good.
...
Now that we have the hash, we can crack using john.![](/src/assets/images/jab/hash.png)john hash --wordlist=/usr/share/wordlists/rockyou.txt![](/src/assets/images/jab/john.png)Creds

[jmontgomery@JAB.HTB](mailto\:jmontgomery@JAB.HTB) : Midnight\_121

We can use this creds on the pidgin client

![](/src/assets/images/jab/mont.png)Follow through with process like we used in creating the previous accounts. Searching for rooms, we see a new room 'pentest2023'

![](/src/assets/images/jab/room.png)Looking at the conversations in the room, we see that someone had ran a GetUserSPN imapact command and has gotten the hash of the svc\_openfire service account. We also have the password that has been cracked using hashcat.

![](/src/assets/images/jab/pass.png)
...
![](/src/assets/images/jab/rpc.png)Ref: [https://book.hacktricks.xyz/network-services-pentesting/135-pentesting-msrpc](https://book.hacktricks.xyz/network-services-pentesting/135-pentesting-msrpc)

How to use dcomexec: [https://wadcoms.github.io/wadcoms/Impacket-DCOMExec/](https://wadcoms.github.io/wadcoms/Impacket-DCOMExec/)

impacket-dcomexec -object MMC20 jab.htb/svc\_openfire\@10.10.11.4

Getting stuck here

![](/src/assets/images/jab/stuck.png)We can pass a command to execute within the victim machine. I will be passwing this as a payload within the dcomexec command. To do this, I have this python script that creates a payload then encodes it in base64 format
...
![](/src/assets/images/jab/python.png)Now we have a base64 encoded payload, we can pass it to our decomexec command. Remember to start a necat cat listener on port 4444 rlwrap nc -nvlp 4444
...
Now we get a shell as svc\_openfire user:![](/src/assets/images/jab/shell.png)
...
To get the exploit in the current working directory. Checking the content, we have details about the exploit and how to become admin. We k![](/src/assets/images/jab/ex.png)We now know that we are dealing with CVE-2022-22733.
...
![](/src/assets/images/jab/port.png)We see that it typically runs on port 9090. For port forwarding, we use chisel. Transfer chisel to windows machine:
...
So now, we will have access to the openfire portal on port 9090![](/src/assets/images/jab/portal.png)Now we can login in with the svc\_openfire credentials. We see the admin console

![](/src/assets/images/jab/admin.png)
...
![](/src/assets/images/jab/ex1.png)login with new user creds and upload jar file as plugin. Dont forget to change the csrf and session token as well

![](/src/assets/images/jab/admin.png)Over here, we see that under the Description tab we wee pass 123. Now, we go to  Server > Server settings > Management Tool. You will be asked for a password, enter: 123

![](/src/assets/images/jab/mgt.png)We now see some details about the server 

![](/src/assets/images/jab/deets.png)We can choose \system command\ to run commands on the server and get interactive shell.

![](/src/assets/images/jab/cmd.png)

Check which user we are:

![](/src/assets/images/jab/NTauth.png)
...
![](/src/assets/images/jab/execute_shell.png)
