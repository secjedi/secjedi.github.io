...
A big shout out to Matej Macek for granting me access to this AWS workshop playground. The workshop is available [here]('https://catalog.workshops.aws/aws-cirt-ransomware-simulation-and-detection/en-US') and can be spined up in different environment like the AWS free-tier env, and followed through.
...
From before..
![activity of unauthorized user](../../assets/images/cloud/aws/ransomware/1.png)
We saw that the  `tdir-workshop-rroe-dev` user can only perform limited funtion as seen Creatbucket, listbucket, tagging, etc. The event by the `tdir-workshop-rroe-dev` user stopped at 	
...
After download, we can open the csv fikele and sort by UsageValue. In this file, we are interested in teh GetObject and DeletecObject Calls; these are 
...
Cloudwatch can also be used to view S3 mterics. Key metrics to look out for are: `DeleteRequests: Sum` and `BytesDownloaded: Sum`. This provide sum of total delete requests and downloaded bytes which are good indicators of ransomware activities.
...
We can search for actitives made to the highly confidential `company-secrets.doc` file using Athena.
```
...
```
We get no result here, indicating that, we cannot say whether the file was accessed or deleted. We dont have evidenceto prove this.
...
We can go to the S3 bucket itself to reonfigure logging
...
- Your key tools for investigation process are Athena to query Cloudtrail logs and GuardDuty
